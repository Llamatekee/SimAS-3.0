...
\begin{enumerate}
    ...
    \item \textbf{sincronizarIdiomaConVentanasSecundarias()}: sincroniza el cambio de idioma con todas las ventanas secundarias activas.
    ...
    \item \textbf{cargarGramaticaYSimularDirectamente()}: proceso completo para cargar gramática y crear simulador.
...
\begin{enumerate}
    ...
    \item \textbf{calcularPosicionSeguaDespuesDelMenu(TabPane)}: calcula posición segura después del menú.
...
\begin{enumerate}
    ...
    \item \textbf{initialize()}: configura la ventana, escena y componentes principales (método privado).
    \item \textbf{configurarAtajosTeclado(Scene)}: establece los atajos de teclado globales (método privado).
    \item \textbf{findFirstTabInGroup(int)}: encuentra la primera pestaña de un grupo específico (método privado).
    ...
    \item \textbf{abrirEditorEnEditorWindow()}: abre un nuevo editor en esta ventana (método privado).
    \item \textbf{abrirSimuladorEnEditorWindow()}: abre el simulador directo en esta ventana (método privado).
    \item \textbf{cargarGramaticaYSimularDirectamenteEnEditorWindow()}: carga gramática y va directo a simulación (método privado).
    \item \textbf{crearSimuladorDirectoAlPaso6EnEditorWindow(Gramatica)}: crea simulador directo al paso 6 (método privado).
    \item \textbf{abrirAyuda()}: abre el manual de ayuda (método privado).
    \item \textbf{abrirTutorial()}: abre el tutorial (método privado).
    \item \textbf{salirAplicacion()}: sale de la aplicación (método privado).
    \item \textbf{mostrarErroresValidacion(ObservableList<String>)}: muestra errores de validación (método privado).
    \item \textbf{mostrarError(String, String)}: muestra mensaje de error simple (método privado).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga la interfaz desde archivo FXML (método privado).
...
\begin{enumerate}
    ...
    \item \textbf{configurarRelacionesPadreHijo()}: configura las relaciones padre-hijo (método privado).
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga la interfaz desde archivo FXML (método privado).
    \item \textbf{initialize()}: método de inicialización FXML.
    \item \textbf{configurarValidaciones()}: configura las validaciones en tiempo real (método privado).
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga la interfaz desde archivo FXML (método privado).
    \item \textbf{initialize()}: método de inicialización FXML.
    \item \textbf{aplicarEstilosResponsivos()}: aplica estilos responsivos según el tamaño de pantalla (método privado).
    \item \textbf{cargarDatosDesdeGramatica()}: carga datos desde la gramática padre (método privado).
    \item \textbf{configurarValidacionSimbolos()}: configura la validación de símbolos (método privado).
...
\begin{enumerate}
    ...
    \item \textbf{configurarValidacionProducciones()}: configura la validación de producciones (método privado).
    \item \textbf{actualizarTextos(ResourceBundle)}: actualiza textos para internacionalización.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga la interfaz desde archivo FXML (método privado).
    \item \textbf{initialize()}: método de inicialización FXML.
    \item \textbf{configurarValidacionSimboloInicial()}: configura la validación del símbolo inicial (método privado).
    \item \textbf{cerrarAsistente()}: cierra el asistente de creación (método privado).
    \item \textbf{actualizarTextos(ResourceBundle)}: actualiza textos para internacionalización.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga la interfaz desde archivo FXML (método privado).
    \item \textbf{initialize()}: método de inicialización FXML.
    \item \textbf{generarBotonesPredefinidos()}: genera botones para símbolos predefinidos (método privado).
    \item \textbf{agregarSimbolo(String)}: agrega un símbolo a la lista (método privado).
...
\begin{enumerate}
    ...
    \item \textbf{cargarFXML()}: carga el archivo FXML correspondiente al paso (método privado).
    \item \textbf{inicializarBotones()}: configura los botones de navegación (método privado).
\end{enumerate}
